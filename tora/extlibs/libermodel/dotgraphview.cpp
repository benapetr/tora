...
//   hide();
    ...
    if (m_detailLevel == 0)      {
        scaleX = m_graph->scale() * 0.7;
        scaleY = m_graph->scale() * 0.7;
    }
    else if (m_detailLevel == 1) {
        scaleX = m_graph->scale() * 1.0;
        scaleY = m_graph->scale() * 1.0;
    }
    else if (m_detailLevel == 2) {
        scaleX = m_graph->scale() * 1.3;
        scaleY = m_graph->scale() * 1.3;
    }
    else                        {
        scaleX = m_graph->scale() * 1.0;
        scaleY = m_graph->scale() * 1.0;
...
//       std::cerr << "Adding graph label '"<<dro.str<<"'" << std::endl;
    ...
    labelViewsIt = m_labelViews.begin();
    labelViewsIt_end = m_labelViews.end();
